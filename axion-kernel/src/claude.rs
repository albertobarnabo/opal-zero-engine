//! Anthropic Claude provider for the Axion Kernel.
//!
//! Uses the Anthropic Messages API (`/v1/messages`).  By default the model is
//! read from `AXION_MODEL` (or falls back to `claude-sonnet-4-5`).
//! Requires the `ANTHROPIC_API_KEY` environment variable.
//!
//! # Provider selection
//! Set `AXION_PROVIDER=claude` to activate this provider in `axion-server`.
//!
//! # Model routing
...
impl ClaudeProvider {
    ...
    pub fn new_with_explicit_key(
        ...
        let text_model = model.unwrap_or_else(|| {
            env::var("AXION_MODEL").unwrap_or_else(|_| "claude-sonnet-4-5".to_string())
        });
        let client = reqwest::Client::builder()
            .timeout(request_timeout())
            .build()
