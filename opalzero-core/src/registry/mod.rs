...
impl Registry {
    ...
    /// 1. Compile-time manifest path (for opalzero-core)
    /// 2. Sibling opalzero-core path (when called from opalzero-server)
    ...
    pub fn init_default() {
        ...
        // Strategy 2: If in opalzero-server, look for opalzero-core/professionals/manifests
        // CARGO_MANIFEST_DIR will be .../opalzero-server, so go up and into opalzero-core
        if compile_time_path.contains("opalzero-server") {
            let manifest_dir = PathBuf::from(compile_time_path)
                .parent()
                .and_then(|p| p.parent()) // Go to workspace root
                .map(|p| p.join("opalzero-core/professionals/manifests"));
