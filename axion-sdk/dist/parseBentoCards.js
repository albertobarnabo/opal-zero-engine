 ...
 */
export function parseBentoCards(state, options) {
    const payload = state.data_payload ?? {};
    const widgets = state.suggested_widgets ?? [];
    const refinedKeys = options?.refinedKeys ?? new Set();
    if (widgets.length > 0) {
        return widgets
            .map((w) => {
            ...
            if (widget === 'MetricCard') {
                ...
                if (typeof raw === 'object' && raw !== null && !Array.isArray(raw)) {
                    props = { title: humanizeKey(key), ...raw };
                }
    ...
    return Object.entries(payload)
        ...
        props: typeof raw === 'object' && raw !== null
            ? { title: humanizeKey(key), ...raw }
            : { value: raw, title: humanizeKey(key) },
    }));
}
function inferWidget(key, value) {
