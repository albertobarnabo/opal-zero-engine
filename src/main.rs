...
use opalzero_core::run_mission;
use clap::{Parser, Subcommand};

#[derive(Parser, Debug)]
#[command(author, version, about = "OpalZero — autonomous AI agent kernel")]
struct Args {
    #[command(subcommand)]
...
enum Command {
    /// Start the OpalZero MCP server on stdin/stdout.
    ///
    ...
    /// automatically has access to all OpalZero tools (web_search, calculator,
...
async fn main() {
    ...
    if let Some(Command::Mcp { action: McpAction::Serve }) = args.command {
        if let Err(e) = OpalZeroMcpServer::new().serve().await {
            eprintln!("OpalZero MCP server error: {}", e);
            std::process::exit(1);
        }
        return;
    }
    ...
    let _intent = match args.intent {
        Some(ref i) => i.clone(),
        None => {
            eprintln!("Usage: opalzero --intent \"<task>\"  OR  opalzero mcp serve");
            std::process::exit(1);
        }
    };
    ...
    tracing::info!("OpalZero Core Heartbeat Started");
