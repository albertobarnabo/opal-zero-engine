//! # OpalZero MCP Server
//!
//! Exposes OpalZero's tool set as a **Model Context Protocol (MCP) server**,
//! allowing any MCP-compatible AI agent (Claude Code, Gemini CLI, Codex, …)
//! to discover and call OpalZero tools without configuration.
//!
//! ## Protocol
//!
...
//! | `tools/list` | Returns all exposed OpalZero tools in MCP schema format |
...
//! ## Tools exposed
//!
//! All general-purpose OpalZero tools are exposed.  Mission-internal tools that
//! have no meaning outside an active OpalZero mission loop are excluded:
//!
//! | Excluded | Reason |
//! |----------|--------|
//! | `finalize_mission_state` | OpalZero-internal: terminates a mission, meaningless standalone |
...
//!
//! ```bash
//! claude mcp add opalzero -- opalzero mcp serve
//! ```
//!
//! After this, every Claude Code session in the project automatically has access
//! to all OpalZero tools (`web_search`, `calculator`, `sqlite_query`, …).
...
/// Convert an OpalZero [`Tool`] to the MCP `inputSchema` JSON object.
///
/// OpalZero's `ToolParameters` is already a valid JSON Schema fragment; we just
...
/// The OpalZero tools exposed over MCP.
///
...
/// An MCP server that exposes OpalZero's tool set over stdin/stdout (JSON-RPC 2.0).
///
...
///
/// ```rust,no_run
/// use opalzero_core::mcp::OpalZeroMcpServer;
///
/// #[tokio::main]
/// async fn main() {
///     OpalZeroMcpServer::new().serve().await.unwrap();
/// }
/// ```
pub struct OpalZeroMcpServer {
...
impl Default for OpalZeroMcpServer {
    fn default() -> Self {
        Self::new()
    }
}

impl OpalZeroMcpServer {
    ...
    pub async fn serve(self) -> Result<(), String> {
        ...
        tracing::info!(session_id = %self.session_id, "OpalZero MCP server started");
        ...
        loop {
            ...
            if n == 0 {
                ...
                tracing::info!("OpalZero MCP server: EOF on stdin — shutting down");
                break;
            }
    ...
    async fn handle_tools_call(&self, _id: Value, params: &Value) -> Result<Value, String> {
        ...
        // MCP sends arguments as a JSON object; OpalZero's execute_tool expects
...
mod tests {
    ...
    #[test]
    fn opalzero_mcp_server_new_has_unique_session_ids() {
        let a = OpalZeroMcpServer::new();
        let b = OpalZeroMcpServer::new();
    ...
    fn handle_initialize_returns_protocol_version() {
        let server = OpalZeroMcpServer::new();
        let result = server.handle_initialize(&Value::Null).unwrap();
        assert_eq!(result["protocolVersion"], "2025-11-25");
        assert_eq!(result["serverInfo"]["name"], "opalzero-mcp");
