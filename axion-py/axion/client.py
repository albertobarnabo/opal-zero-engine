...
class AxionError(Exception):
    """Raised when the Axion server returns a non-2xx response."""
...
class AxionClient:
    """
    Async client for the Axion Intelligence Kernel API.
    ...
    :class:`httpx.AsyncClient` is properly closed::
        ...
        async with AxionClient(base_url="http://localhost:8000") as axion:
            async for event in axion.execute("Analyse the EV market"):
    ...
    def _client(self) -> httpx.AsyncClient:
        ...
        if self._http is None:
            raise RuntimeError(
                "AxionClient must be used as an async context manager: "
                "`async with AxionClient(...) as axion:`"
            )
        return self._http
    ...
    async def _raise_for_error(self, res: httpx.Response) -> None:
        ...
        try:
            ...
            pass
        raise AxionError(message, code, res.status_code)
