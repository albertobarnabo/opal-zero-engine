...
**Premium OpenAI provider and structured Governor for production OpalZero deployments.**

opalzero-core ships with a built-in provider and a lightweight Governor that are great for development and self-hosted setups. opalzero-kernel upgrades both for production: a hardened OpenAI integration with vision support, and a Governor that injects the full mission context — all task results, the prior payload, and the ContextBus — into its quality evaluation prompt, making it significantly more accurate at catching coverage gaps and structural issues.
...
Production-grade LLM provider backed by `gpt-4o-mini` for text tasks and `gpt-4o` for anything requiring vision. Configures timeouts, token limits, and temperature from environment variables — no code changes needed between environments. Implements the `AiProvider` trait from opalzero-core, so it's a drop-in replacement for `SimpleProvider`.
...
opalzero-kernel activates automatically when `OPENAI_API_KEY` is present. Use it via opalzero-server (which imports it directly), or wire it into your own binary:

```rust
use opalzero_core::prelude::*;
use opalzero_kernel::prelude::{OpalZeroGovernor, OpenAIProvider};
...
async fn main() {
    ...
    let governor = OpalZeroGovernor::new();
...
```toml
[dependencies]
opalzero-kernel = { git = "https://github.com/albertobarnabo/opalzero-kernel" }
...
Inherits all env vars from opalzero-core. No additional variables required.
...
| `OPALZERO_MISSION_TIMEOUT_SECS` | Wall-clock deadline |

---

## Docs

[albertobarnabo.it/opalzero/docs](https://albertobarnabo.it/opalzero/docs)
