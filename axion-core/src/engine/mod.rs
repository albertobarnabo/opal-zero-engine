...
/// When Axion runs a multi-task mission with several concurrent agents it can
...
///
/// ```rust,no_run
/// use axion_core::engine::{RotatingProvider, SimpleProvider};
///
...
impl RotatingProvider {
    ...
    ///
    /// ```rust,no_run
    /// use axion_core::engine::RotatingProvider;
...
///
/// From Axion's perspective the difference is invisible: both implement
...
///   browsing, code execution) rather than Axion's tool set.  Axion's
...
///
/// ```rust,no_run
/// use axion_core::engine::SubprocessProvider;
///
...
impl AiProvider for SubprocessProvider {
    /// Runs the subprocess agent with the full Axion prompt (system context +
    ...
    /// switching is not supported at the Axion level.
    ///
...
mod tests {
    ...
    fn subprocess_provider_with_text_model_returns_none() {
        // Subprocess agents manage their own model — Axion should not
    ...
    async fn subprocess_provider_errors_on_missing_binary() {
        let p = SubprocessProvider::new("__axion_nonexistent_binary_xyz__");
