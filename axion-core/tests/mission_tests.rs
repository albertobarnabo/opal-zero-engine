...
use axion_core::protocol::{AgentRole, MissionUpdate, Tool, WasmPreopen};
use axion_core::{resume_mission, run_mission};
...
async fn execute_tool_dispatches_calculator_via_wasm() {
    ...
    let result = axion_core::tools::execute_tool(
        ...
        "test",
        &axion_core::tools::RequestKeys::default(),
    )
    .await;
    ...
    if !wasm_path.exists() {
        eprintln!(
            "SKIP wasm_memory_recalls_snapshot_by_id — {} not found.\n\
             Build it with:\n  \
             cd axion-professionals/memory && \
             cargo build --target wasm32-wasip1 --release\n  \
             cp target/wasm32-wasip1/release/axion-memory.wasm \
             ../../axion-core/professionals/memory.wasm",
            MEMORY_WASM
        );
        return;
    }
...
async fn execute_tool_dispatches_memory_via_wasm() {
    ...
    let args = format!(r#"{{"mission_id":"{}"}}"#, TEST_SNAPSHOT_ID);
    let result = axion_core::tools::execute_tool("memory", &args, "test", &axion_core::tools::RequestKeys::default()).await;
